...
ahs_climate_excl = (
    ahs_climate_raw
    # utils cost flags
    .filter(~((col("TENURE") == 2) & (col("UTILAMT") == 0))) # exclude renters who don't explicitely pay utilities
    .filter(~((col("TENURE") == 1) & (col("UTILAMT") == 0))) # exclude homeowners with 0 utility cost
    ...
    .filter(col("UTILAMT") * 12 <= col("HINCP")) # Impossible
